...
class InstantOrderGenerate extends Command
{
    ...
    public function fire()
    {
        ...
        $isExisted = InstantOrder::where(function(Builder $builder) use($dates, $hallIds, $courtIds){
            if (count($hallIds) > 0) {
        ...
        $validHalls = Court::where(function (Builder $builder) use ($hallIds, $courtIds) {
            if (count($hallIds) > 0) {
                $builder->getQuery()->whereIn('hall_id', $hallIds);
            }
            if (count($courtIds) > 0) {
                $builder->getQuery()->whereIn('id', $courtIds);
            }
        })->groupBy('hall_id')->get(array('hall_id'));

        $validHallIds = array();
        foreach ($validHalls as $validHall) {
            $validHallIds[] = $validHall->hall_id;
        }
        ...
        HallMarket::with(
            ...
            ->whereIn('hall_id', $validHallIds)
            ->chunk(10, function ($hallMarkets) use ($dateTimestamps, $courtIds) {
