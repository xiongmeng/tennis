...
class HallImageHelper extends Command
{
     ...
     * @var string
     */
    protected $description = "dispose image for hall.";
    ...
    {
        //8991
        $imageRootPath = '/var/www/gotennis/web';
        $argument = $this->argument(self::ARGUMENT_OPERATE);

        $hallIdStrings = $this->option(self::OPTION_HALL);
        $hallIds = explode(',', $hallIdStrings);
//        $this->info('dispose hallIds: '. print_r($hallIds, true));
        switch($argument){
            case 'thumb':
                Hall::with('HallImages', 'Envelope')->whereIn('id', $hallIds)->chunk(10, function($halls) use($imageRootPath) {
                    foreach($halls as $hall) {
                        if($hall instanceof Hall){
                            $this->info("dispose hall $hall->id ($hall->name)");
                            $hallHead = hall_head($hall);
                            if($hallHead){
                                $srcPath = $imageRootPath . $hallHead->path;
                                $desPath =  "$imageRootPath/Images/weixinImage/CourtPic/$hall->id.jpg";
                                $this->info('src image : ' . $srcPath);
                                $this->info('des image : ' . $desPath);
                                $image = new ResizeImage($srcPath, 200, 200, 0, $desPath);
                            }
                        }
                    }
                });
    ...
    protected function getArguments()
    {
        return array(
            array(self::ARGUMENT_OPERATE, InputArgument::OPTIONAL,
                sprintf(
                    "%s",
                    "thumb - generate thumbnail for hall"
                ))
        );
    }
    ...
    protected function getOptions()
    {
        return array(
            array(self::OPTION_HALL, null,
                InputOption::VALUE_OPTIONAL, "the hall ids, join with ','", null),
        );
    }
