...
class SeekingOrder extends Eloquent implements \Finite\StatefulInterface
{
    ...
    protected $fillable = array('state', 'seeking_id', 'seeker', 'joiner', 'cost', 'hall_id', 'expire_time',
    ...
    public function createFromSeeking(Seeking $seeking, $joiner)
    {
        return self::create(array('state' => SEEKING_ORDER_STATE_DISPOSING, 'seeking_id' => $seeking->id,
            'hall_id' => $seeking->hall_id, 'event_date' => $seeking->event_date, 'start_hour' => $seeking->start_hour,
            'end_hour' => $seeking->end_hour, 'seeker' => $seeking->creator, 'joiner' => $joiner,
            'cost' => $seeking->personal_cost));
    }
