...
class User extends Eloquent implements UserInterface, RemindableInterface
{
    ...
    public function search($aQuery, $iPageSize = 20, $relations = array())
    {
        is_string($relations) && ($relations = (strlen($relations) > 0) ? explode(',', $relations) : array());

        $query = User::with($relations)->leftJoin('gt_account', function (\Illuminate\Database\Query\JoinClause $join) {
